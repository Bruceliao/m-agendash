...
module.exports = agendash => {
  ...
  app.post('/api/jobs/delete', (req, res) => {
    ...
    agendash.deleteJobs(req.body.jobIds, err => {
      ...
      console.log('deleting job ids', req.body.jobIds)
  ...
  app.post('/api/jobs/create/url-task', (req, res) => {
    ...
    agendash.createJob("URL", req.body.jobSchedule, req.body.jobRepeatEvery, req.body.jobData, req.body.taskName, (err, doc) => {
      console.log(err, doc, "create job")
      if (err) {
        return res.status(404).json(err);
      }
      return res.json({created: true, jobId: doc.attrs._id});
    });
  });
